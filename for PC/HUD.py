...
def draw_text_rotated(text, x, y, angle_deg, font=font):
...
def draw_horizon_lines(pitch, roll, yaw):
    for deg in range(-180, 180, 5):
        ...
        if deg > 0:
            ...
            small_gap1_position = (1 / 7)
            small_gap2_position = 1 - small_gap1_position

            p1x = x1_after_rotate + dx * small_gap1_position # left small gap center position
            p1y = y1_after_rotate + dy * small_gap1_position # left small gap center position
            p2x = x1_after_rotate + dx * small_gap2_position # right small gap center position
            p2y = y1_after_rotate + dy * small_gap2_position # right small gap center position
            ...
            pygame.draw.line(screen, GREEN, (p1x + ox_small, p1y + oy_small), (x_center_after_rotate - ox, y_center_after_rotate - oy), 1)
            # Draw third line (between middle gap and right small gap)
            pygame.draw.line(screen, GREEN, (x_center_after_rotate + ox, y_center_after_rotate + oy), (p2x - ox_small, p2y - oy_small), 1)
            # Draw fourth line (right side of right small gap)
            pygame.draw.line(screen, GREEN, (p2x + ox_small, p2y + oy_small), (x2_after_rotate, y2_after_rotate), 1)
        ...
        if deg > 0:
            ...
            draw_text_rotated(f"{-deg}", int(text_x_left), int(text_y_left), -text_angle)
            ...
            draw_text_rotated(f"{-deg}", int(text_x_right), int(text_y_right), -text_angle)
...
def draw_altmeter(alt):
    ...
    for alt_tick in range(start_alt, int(alt) + 70, 10):
        ...
        if not (tick_y < alt_center_y + 10 and tick_y > alt_center_y - 3):
            draw_text(f"{alt_tick}", alt_center_x + 8, int(tick_y - 2), align="left", font=font_small)

# Draw 3d speed line function
def draw_speedmeter(speed_3d):
    speed_center_x = CENTER_X - 38
...
def render_hud(pitch, roll, yaw, v_speed, alt, speed_3d, sats, course, vbat, current, home_dist, home_dir):
        ...
        draw_text(f"{int(speed_3d)}", CENTER_X - 48, CENTER_Y, align="right")

        # Draw altitude meter (right side of HUD)
        draw_altmeter(alt)

        # Altitude text (right side of HUD)
        draw_text(f"{int(alt)}", CENTER_X + 46, CENTER_Y, align="left")

        # Info texts (bottom right side of HUD)
        draw_text(f"BAT {vbat:.1f}V", CENTER_X + 35, CENTER_Y + 25, align="left", font=font_small)
        draw_text(f"CUR {current:.1f}A", CENTER_X + 35, CENTER_Y + 30, align="left", font=font_small)
        draw_text(f"HOME {home_dist}m", CENTER_X + 35, CENTER_Y + 35, align="left", font=font_small)
        draw_text(f"HOME {home_dir}°", CENTER_X + 35, CENTER_Y + 40, align="left", font=font_small)
