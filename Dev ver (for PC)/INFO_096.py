...
# Draw moving parts
def render_info(vbat, current, rssi, throttle):
...
def main(msp_data):
    ...
    while True:
        ...
        rssi = msp_data["rssi"] if msp_data["rssi"] is not None else 0.0
        throttle = msp_data["throttle"] if msp_data["throttle"] is not None else 0.0

        # Render dynamic parts of INFO
        render_info(vbat, current, rssi, throttle)
